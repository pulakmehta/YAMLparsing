...
class Test(unittest.TestCase):
    ...
    def test_given_yaml_lists(self):
        test1 = os.path.join(dirname,'TestCase/2/test','test2.yaml')
        test1_ans = os.path.join(dirname,'TestCase/expected','2.yaml')
        with open(test1_ans) as file:
            documents = yaml.load(file, Loader=yaml.FullLoader)
        self.assertEqual(MergingScript(test1).main(), documents)

    def test_given_nested_key_value(self):
        test1 = os.path.join(dirname,'TestCase/test','test3.yaml')
    ...
    def test_empty_file_inner(self):
        test1 = os.path.join(dirname,'TestCase/6/test/test','test6.yaml')
        test1_ans = os.path.join(dirname,'TestCase/expected','6.yaml')
        with open(test1_ans) as file:
            documents = yaml.load(file, Loader=yaml.FullLoader)
        self.assertEqual(MergingScript(test1).main(), documents)
